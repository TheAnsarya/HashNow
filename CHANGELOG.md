...
- **GUI dialogs not appearing** — MessageBox dialogs now use a topmost owner form so they always appear in front of other windows instead of hiding behind them
- **Console output broken** — WinExe mode now properly redirects .NET Console streams after `AttachConsole`, fixing silent failures when run from terminal
- **WinForms initialization** — added `Application.EnableVisualStyles()`, `SetHighDpiMode()`, and `SetCompatibleTextRenderingDefault()` before any GUI dialog is shown

### Added

- **Uninstall instructions** in README — documents how to remove context menu and upgrade from older versions
